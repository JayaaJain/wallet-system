...
router.get('/transactions', async (req, res) => {
    ...
    if(!wallet || !wallet.id) {
        const transactions = await Transaction.find({}).select({_id: 0, __v: 0}).skip(skip).limit(limit);
        return res.send(transactions);
    }
    const transactions = await Transaction.find({walletId: givenWalletId}).select({_id: 0, __v: 0}).skip(skip).limit(limit);
    return res.send(transactions);
});
